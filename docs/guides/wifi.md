...
In order to check if that's the case, run `iwctl station list`. 
If no devices show up, then the driver is not correctly loaded.
Go to the beginning of this guide and double-check if you
...
Now, you can change your wifi security settings:

1. Connect to your network from macOS or another working device.

2. Find your wifi security setting and change it from `WPA2-PSK`
to `WPA-PSK` or `WPA2-PSK + WPA-PSK`.
> Warning: This step makes your wifi network less secure.
Consider if this is worth it to you.
